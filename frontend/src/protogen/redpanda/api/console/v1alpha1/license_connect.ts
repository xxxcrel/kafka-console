// @generated by protoc-gen-connect-es v1.2.0 with parameter "target=ts,import_extension="
// @generated from file redpanda/api/kconsole/v1alpha1/license.proto (package redpanda.api.kconsole.v1alpha1, syntax proto3)
/* eslint-disable */
// @ts-nocheck

import { ListEnterpriseFeaturesRequest, ListEnterpriseFeaturesResponse, ListLicensesRequest, ListLicensesResponse, SetLicenseRequest, SetLicenseResponse } from "./license_pb";
import { MethodKind } from "@bufbuild/protobuf";

/**
 * @generated from service redpanda.api.kconsole.v1alpha1.LicenseService
 */
export const LicenseService = {
  typeName: "redpanda.api.kconsole.v1alpha1.LicenseService",
  methods: {
    /**
     * ListLicenses lists all the roles based on optional filter.
     *
     * @generated from rpc redpanda.api.kconsole.v1alpha1.LicenseService.ListLicenses
     */
    listLicenses: {
      name: "ListLicenses",
      I: ListLicensesRequest,
      O: ListLicensesResponse,
      kind: MethodKind.Unary,
    },
    /**
     * SetLicense installs a new license on the Redpanda cluster.
     * This endpoint only works if the Redpanda Admin API is configured.
     *
     * @generated from rpc redpanda.api.kconsole.v1alpha1.LicenseService.SetLicense
     */
    setLicense: {
      name: "SetLicense",
      I: SetLicenseRequest,
      O: SetLicenseResponse,
      kind: MethodKind.Unary,
    },
    /**
     * ListEnterpriseFeatures reports the license status and Redpanda enterprise features in use.
     * This can only be reported if the Redpanda Admin API is configured and supports this request.
     *
     * @generated from rpc redpanda.api.kconsole.v1alpha1.LicenseService.ListEnterpriseFeatures
     */
    listEnterpriseFeatures: {
      name: "ListEnterpriseFeatures",
      I: ListEnterpriseFeaturesRequest,
      O: ListEnterpriseFeaturesResponse,
      kind: MethodKind.Unary,
    },
  }
} as const;

