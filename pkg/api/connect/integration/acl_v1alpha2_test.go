...
import (
	...
	"github.com/xxxcrel/kafka-console/pkg/protocmp"
	v1alpha2 "github.com/xxxcrel/kafka-console/pkg/protogen/redpanda/api/dataplane/v1alpha2"
	v1alpha2connect "github.com/xxxcrel/kafka-console/pkg/protogen/redpanda/api/dataplane/v1alpha2/dataplanev1alpha2connect"
)

func (s *APISuite) DeleteAllACLs_v1alpha2(ctx context.Context) error {
	acls := kadm.NewACLs().
		Allow().
		Deny().
		AnyResource().
...
func (*APISuite) newStdACLResource_v1alpha2(resourceType v1alpha2.ACL_ResourceType, resourceName string, principal string) *v1alpha2.ListACLsResponse_Resource {
	return &v1alpha2.ListACLsResponse_Resource{
		ResourceType:        resourceType,
		ResourceName:        resourceName,
		ResourcePatternType: v1alpha2.ACL_RESOURCE_PATTERN_TYPE_LITERAL,
		Acls: []*v1alpha2.ListACLsResponse_Policy{
			{
				Principal:      principal,
				Host:           "*",
...
func (s *APISuite) TestCreateACL_v1alpha2() {
	t := s.T()
	...
	t.Run("create ACL with default request (connect-go)", func(t *testing.T) {
		...
		createReq := &v1alpha2.CreateACLRequest{
	...
	t.Run("create ACL with default request (http)", func(t *testing.T) {
		...
		err := requests.
			URL(s.httpAddress() + "/v1alpha2/acls").
			BodyJSON(&httpReq).
			Post().
...
func (s *APISuite) TestListACLs_v1alpha2() {
	t := s.T()
	...
	t.Run("list ACLs with default request (connect-go)", func(t *testing.T) {
		...
		resourceNames := map[v1alpha2.ACL_ResourceType]string{
			...
			v1alpha2.ACL_RESOURCE_TYPE_TOPIC:            fmt.Sprintf("%vtopic", resourceNamePrefix),
			v1alpha2.ACL_RESOURCE_TYPE_TRANSACTIONAL_ID: fmt.Sprintf("%vtransactional-id", resourceNamePrefix),
			v1alpha2.ACL_RESOURCE_TYPE_CLUSTER:          "kafka-cluster",
		}

		acls := kadm.NewACLs().
			Allow(principal).
			ResourcePatternType(kadm.ACLPatternLiteral).
			Groups(resourceNames[v1alpha2.ACL_RESOURCE_TYPE_GROUP]).
			Topics(resourceNames[v1alpha2.ACL_RESOURCE_TYPE_TOPIC]).
			TransactionalIDs(resourceNames[v1alpha2.ACL_RESOURCE_TYPE_TRANSACTIONAL_ID]).
			Clusters().
			Operations(kadm.OpAll)
		...
		// 2. List ACLs
		client := v1alpha2connect.NewACLServiceClient(http.DefaultClient, s.httpAddress())
		res, err := client.ListACLs(ctx, connect.NewRequest(&v1alpha2.ListACLsRequest{}))
		require.NoError(err)
		...
		filteredResources := make([]*v1alpha2.ListACLsResponse_Resource, 0)
		...
		expectedResources := map[v1alpha2.ACL_ResourceType]*v1alpha2.ListACLsResponse_Resource{
			v1alpha2.ACL_RESOURCE_TYPE_TOPIC:            s.newStdACLResource_v1alpha2(v1alpha2.ACL_RESOURCE_TYPE_TOPIC, resourceNames[v1alpha2.ACL_RESOURCE_TYPE_TOPIC], principal),
			v1alpha2.ACL_RESOURCE_TYPE_TRANSACTIONAL_ID: s.newStdACLResource_v1alpha2(v1alpha2.ACL_RESOURCE_TYPE_TRANSACTIONAL_ID, resourceNames[v1alpha2.ACL_RESOURCE_TYPE_TRANSACTIONAL_ID], principal),
			v1alpha2.ACL_RESOURCE_TYPE_GROUP:            s.newStdACLResource_v1alpha2(v1alpha2.ACL_RESOURCE_TYPE_GROUP, resourceNames[v1alpha2.ACL_RESOURCE_TYPE_GROUP], principal),
			v1alpha2.ACL_RESOURCE_TYPE_CLUSTER:          s.newStdACLResource_v1alpha2(v1alpha2.ACL_RESOURCE_TYPE_CLUSTER, resourceNames[v1alpha2.ACL_RESOURCE_TYPE_CLUSTER], principal),
		}
	...
	t.Run("list ACLs with invalid filter (connect-go)", func(t *testing.T) {
		...
		client := v1alpha2connect.NewACLServiceClient(http.DefaultClient, s.httpAddress())
		_, err := client.ListACLs(ctx, connect.NewRequest(&v1alpha2.ListACLsRequest{
			Filter: &v1alpha2.ListACLsRequest_Filter{
				ResourceType:   v1alpha2.ACL_RESOURCE_TYPE_ANY,
				PermissionType: v1alpha2.ACL_PermissionType(999),
			},
		}))
		assert.Error(err)
...
func (s *APISuite) TestDeleteACLs_v1alpha2() {
	t := s.T()
	...
	t.Run("delete ACLs with a filter that matches all three created ACLs (connect-go)", func(t *testing.T) {
		...
		deleteRequest := &v1alpha2.DeleteACLsRequest{
	...
	t.Run("delete ACLs with invalid filter (connect-go)", func(t *testing.T) {
		...
		client := v1alpha2connect.NewACLServiceClient(http.DefaultClient, s.httpAddress())
		_, err := client.DeleteACLs(ctx, connect.NewRequest(&v1alpha2.DeleteACLsRequest{
			Filter: &v1alpha2.DeleteACLsRequest_Filter{
				ResourceType:   v1alpha2.ACL_RESOURCE_TYPE_ANY,
				PermissionType: v1alpha2.ACL_PermissionType(999),
			},
