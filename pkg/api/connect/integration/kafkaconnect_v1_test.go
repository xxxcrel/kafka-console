...
import (
	...
	v1connect "github.com/xxxcrel/kafka-console/pkg/protogen/redpanda/api/dataplane/v1/dataplanev1connect"
	"github.com/xxxcrel/kafka-console/pkg/testutil"
)

func (s *APISuite) TestListConnectors_V1() {
	t := s.T()
	...
	t.Run("list connectors with default request (connect-go)", func(t *testing.T) {
		...
		assert.IsType([]*v1.ListConnectorsResponse_ConnectorInfoStatus{}, res.Msg.Connectors)
	})
}

func (s *APISuite) TestListConnectClusters_v1() {
	t := s.T()
...
func (s *APISuite) TestGetConnectorAndStatus_v1() {
	t := s.T()
