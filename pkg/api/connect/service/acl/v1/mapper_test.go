...
func TestAclFilterToKafka(t *testing.T) {
	...
	emptyDefaultRequest := &v1.ListACLsRequest{}
...
func TestDescribeACLsResourceToProto(t *testing.T) {
	tests := []struct {
		...
		want      *v1.ListACLsResponse_Resource
		wantError bool
	}{
		{
			name: "basic and complete conversion without error",
			...
			want: &v1.ListACLsResponse_Resource{
				ResourceType:        v1.ACL_RESOURCE_TYPE_TOPIC,
				ResourceName:        "test",
				ResourcePatternType: v1.ACL_RESOURCE_PATTERN_TYPE_MATCH,
...
func TestAclOperationToKafka(t *testing.T) {
	tests := []struct {
		name      string
		input     v1.ACL_Operation
		want      kmsg.ACLOperation
		wantError bool
	}{
		{"any", v1.ACL_OPERATION_ANY, kmsg.ACLOperationAny, false},
		{"all", v1.ACL_OPERATION_ALL, kmsg.ACLOperationAll, false},
		{"read", v1.ACL_OPERATION_READ, kmsg.ACLOperationRead, false},
		{"unknown", v1.ACL_Operation(9999), kmsg.ACLOperationUnknown, true}, // testing an unknown operation
	}
