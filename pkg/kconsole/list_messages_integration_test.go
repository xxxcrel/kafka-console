...
func (s *ConsoleIntegrationTestSuite) TestListMessages() {
	...
	t.Run("metadata topic error", func(t *testing.T) {
		...
		err = svc.ListMessages(ctx, input, mockProgress)
		assert.Error(err)
		assert.Equal("failed to get metadata for topic test.redpanda.kconsole.list_messages: LEADER_NOT_AVAILABLE: There is no leader for this topic-partition as we are in the middle of a leadership election.", err.Error())
	})
